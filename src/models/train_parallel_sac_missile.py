...
def make_env(
    ...
    def _f():
        ...
        env_ = Interception2D(
            ...
            scenarios=[],
            model=target_model,
            missile_reward_params=missile_reward_params
        )
        env_.seed(seed)
        return env_
    return _f
...
def train(
    ...
    stack = [make_env(seed, missile_reward_params=missile_reward_params) for seed in range(N_PROC)]
    spv_env = SubprocVecEnv(stack, start_method='spawn')

    with open(os.path.join(BASE_PATH, 'src', 'models', 'hyperparams', 'sac_missile.yaml')) as f:
